				...
				if( (mymap_points[a] != mymap_couples[i]->point1)  && ( mymap_points[a] != mymap_couples[i]->point2) )
				{
					...
					if(tracking >= 2 && a == (mymap_points.size() - 1)) // IF TRACKING >=2 AND IT IS THE END OF THE LOOP IT PRINTS THE SEQUENCE OF COLLINEAR POINTS
						...
						mymap_couples[i]->print(); // PRINTS THE COUPLES
							...
							for( int i = 0 ; i < mymap_container.size() ; i++) // PRINTS THE POINTS COLLINEARS TO THE COUPLE
							{	
...
// We disregarded reading in the files characters as being the detemeriner of n, because the number points are what are being input into the collinear line finder function.
//The for loop that creates the couples runs in (n^2-n) because it is combination of n choose 2.
//The other major loop runs in (1/2)((n^2 )-n), because it is derived from the sum from 1 to n of n – k, which represents everytime a collinear pair is found the number of points that can be part of the collinear line reduces by one.
//The runtime T(n) = ((n^2) – n) + (1/2)((n^2) -n) = (3/2)((n^2) - n)
