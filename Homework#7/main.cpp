...
vector<int> * make_adj_list(string file_name);
void print_arr(vector<int> *arr, int len_arr);

int main() {

    string file = "matrix6x6.txt";                       //file 

    vector<int> *adj_list_p = make_adj_list(file);  //make adjacency list (pointer to array of int vectors.)

    int num_v = adj_list_p[0][0];                   //number of vertex

       cout << " the number of vertex is " << num_v << endl ;                                               //print adjacency list
Graph g(num_v + 1);
int k = 1;
